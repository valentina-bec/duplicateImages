...
preparation of the img_data
 - go trouht directories
 - extract files to check
 - convert img into a np.array
    - resize 
    - grayscale
- save the dataset : array and file as label
...
def select_imgs(src):
    ...
    """
    # if dir, extract the fotos
    if os.path.isdir(src):
        #l_path = list_img_path(src)
        return list_img_path(src) #l_path # list files
    #if source is only a file:
    elif os.path.isfile(src) and imghdr.what(src) != 'gif':
        return [src]
    else: print ("directory or file not acepted, please select an other one!") #Error Value
...
def main():
    #select dir or foto so convert into array
    dir0 = askdirectory()
